...
TEST(init, testInit) { ASSERT_EQ(10, 10); }
...
TEST(list_moves, clean_board) {
  ...
  std::list<field> four_visited = {{0, 0}, {2, 1}, {4, 2}, {2, 3}, {4, 4},
                                   {3, 2}, {1, 3}, {3, 4}, {2, 2}, {4, 3}};
  for (field v : four_visited) b5(v.get_letter(), v.get_num(), true);
  std::list<field> four_already_looked = {{2, 4}};
  ASSERT_EQ(fouth_check,
            list_available_kight_moves(b5, {4, 3}, four_already_looked));
}
TEST(validate_moves, moves) {
  int max_letter = 5;
  int max_num = 5;
  std::list<field> right_path = {
      {0, 0}, {1, 2}, {0, 4}, {2, 3}, {4, 4}, {3, 2}, {4, 0}, {2, 1}, {0, 2},
      {1, 4}, {3, 3}, {4, 1}, {2, 0}, {0, 1}, {1, 3}, {3, 4}, {4, 2}, {3, 0},
      {1, 1}, {0, 3}, {2, 4}, {4, 3}, {3, 1}, {1, 0}, {2, 2}};
  ASSERT_TRUE(validate_moves(right_path, max_letter, max_num));
  std::list<field> double_path = {
      {0, 0}, {1, 2}, {0, 4}, {2, 3}, {4, 4}, {3, 2}, {4, 0}, {2, 1}, {0, 2},
      {1, 4}, {3, 3}, {4, 1}, {2, 0}, {0, 1}, {1, 3}, {3, 4}, {4, 2}, {3, 1},
      {1, 1}, {0, 3}, {2, 4}, {4, 3}, {3, 1}, {1, 0}, {2, 2}};
  ASSERT_FALSE(validate_moves(double_path, max_letter, max_num));
  std::list<field> long_path = {
      {0, 0}, {1, 2}, {0, 4}, {2, 3}, {4, 4}, {3, 2}, {4, 0}, {2, 1}, {0, 2},
      {1, 4}, {3, 3}, {4, 1}, {2, 0}, {0, 1}, {1, 3}, {3, 4}, {4, 2}, {3, 0},
      {1, 1}, {0, 3}, {2, 4}, {4, 3}, {3, 1}, {1, 0}, {2, 2}, {0, 0}};
  ASSERT_FALSE(validate_moves(long_path, max_letter, max_num));
  std::list<field> short_path = {
      {0, 0}, {1, 2}, {0, 4}, {2, 3}, {4, 4}, {3, 2}, {4, 0}, {2, 1},
      {0, 2}, {1, 4}, {3, 3}, {4, 1}, {2, 0}, {0, 1}, {1, 3}, {3, 4},
      {4, 2}, {3, 0}, {1, 1}, {0, 3}, {2, 4}, {4, 3}, {3, 1}, {1, 0}};
...
TEST(get_solution, solve_simple5) {
  ...
  int max_num = 5;
  board b(max_letter, max_num);
  ASSERT_TRUE(validate_moves(solve_template<chooseForce>(b, {0, 0}), max_letter,
                             max_num));
}
TEST(get_solution, solve_simple7) {
  int max_letter = 7;
  int max_num = 7;
  board b(max_letter, max_num);
  ASSERT_TRUE(validate_moves(solve_template<chooseForce>(b, {0, 0}), max_letter,
...
#ifdef TOO_LONG
TEST(get_solution, solve_simple8) {
  int max_letter = 8;
  int max_num = 8;
  board b(max_letter, max_num);
  ASSERT_TRUE(validate_moves(solve_template<chooseForce>(b, {0, 0}), max_letter,
...
TEST(get_solution, solve_v5) {
  int max_letter = 5;
  int max_num = 5;
  board b(max_letter, max_num);
  ASSERT_TRUE(validate_moves(solve_template<chooseWarnsdorff>(b, {0, 0}),
                             max_letter, max_num));
}
TEST(get_solution, solve_v7) {
  int max_letter = 7;
  int max_num = 7;
  board b(max_letter, max_num);
  ASSERT_TRUE(validate_moves(solve_template<chooseWarnsdorff>(b, {0, 0}),
