...
int Knapsack::Bound(const BABNode &node) {
  if (node.elem.weight >= capacity_) return 0;

  int profit_bound = node.elem.profit;
  int j = node.level + 1;
  int total_weight = node.elem.weight;

  while ((j < elements_.size()) &&
  ...
  if (j < elements_.size()) {
    profit_bound +=
        (capacity_ - total_weight) * elements_[j].profit / elements_[j].weight;
  }
...
size_t Knapsack::BranchAndBound() {
  ...
  while (!Q.empty()) {
    ...
    if (u.level == elements_.size() - 1) continue;
